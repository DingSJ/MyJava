...
public class PeopleDeep implements Cloneable{
    ...
    public PeopleDeep getFriend() {
        return friend;
    }

    public void setFriend(PeopleDeep friend) {
        this.friend = friend;
    }
    ...
    @Override
    protected PeopleDeep clone() throws CloneNotSupportedException {
        PeopleDeep people = null;
        try {
            people = (PeopleDeep) super.clone();

            List<Demo> demos = this.getDemos().stream().collect(Collectors.toList());
            people.setDemos(demos);
            if (this.getHobby() != null) {
                people.setHobby(this.getHobby().stream().collect(Collectors.toList()));
            }
            if (this.getFriend() != null) {
                people.setFriend(this.getFriend().clone());
            }
            if (this.getDemos() != null) {
                people.setDemos(this.getDemos().stream().collect(Collectors.toList()));
            }
